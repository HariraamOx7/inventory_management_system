// frontend/src/pages/ProdHeadMaster.jsx
import { useState, useEffect, useMemo } from 'react';
import axios from 'axios';
import {
  Plus, Edit2, Trash2, Save, X, Package, AlertCircle, Search, ArrowUpDown,
...
export default function ProductHead() {
  ...
  const fetchProductHeads = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${API_URL}/product-heads`);
      if (response.data?.success) {
        setProductHeads(response.data.data);
      }
    } catch (error) {
      setError('Error fetching product heads: ' + error.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredProductHeads = useMemo(() => {
    return productHeads.filter(ph =>
      search.trim() === '' ||
      String(ph.ProdHeadCode).toLowerCase().includes(search.toLowerCase()) ||
      (ph.ProdHeadDesc && ph.ProdHeadDesc.toLowerCase().includes(search.toLowerCase()))
    );
  }, [productHeads, search]);

  const totalItems = filteredProductHeads.length;
  const totalPages = Math.ceil(totalItems / itemsPerPage) || 1;
  const paginatedProductHeads = useMemo(() => {
    const start = (currentPage - 1) * itemsPerPage;
    return filteredProductHeads.slice(start, start + itemsPerPage);
  }, [filteredProductHeads, currentPage, itemsPerPage]);

  const handleAddProductHead = async (e) => {
    e.preventDefault();
    try {
      setLoading(true);
      await axios.post(`${API_URL}/product-heads`, formData);
      setFormData(initialFormState);
      setShowAddForm(false);
      fetchProductHeads();
    } catch (error) {
      setError('Error adding product head: ' + error.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEdit = (ph) => {
    setEditingCode(ph.ProdHeadCode);
    setEditFormData(ph);
  };
  ...
  const handleUpdate = async (code) => {
    try {
      setLoading(true);
      await axios.put(`${API_URL}/product-heads/${code}`, editFormData);
      setEditingCode(null);
      fetchProductHeads();
    } catch (error) {
      setError('Error updating product head: ' + error.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (code) => {
    if (!window.confirm('Are you sure you want to delete this product head?')) return;
    try {
      setLoading(true);
      await axios.delete(`${API_URL}/product-heads/${code}`);
      fetchProductHeads();
    } catch (error) {
      setError('Error deleting product head: ' + error.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Layout>
      <div className="relative p-6 max-w-[1600px] mx-auto space-y-6">
        {/* Header Section */}
        <PageHeader
          title="Product Head Master"
          subtitle="Manage top-level product category heads and classification"
          icon={Package}
          actionText="Add New Product Head"
        ...
        {showAddForm && (
          <div className="bg-white rounded-2xl shadow-xl border border-slate-200 p-6 space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h2 className="text-base font-bold text-slate-800">Add New Product Head</h2>
              <button onClick={() => setShowAddForm(false)} className="text-slate-400 hover:text-slate-600">
                <X className="w-5 h-5" />
              </button>
            </div>
            <form onSubmit={handleAddProductHead} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="text"
                    required
                    value={formData.ProdHeadCode}
                    onChange={(e) => setFormData({ ...formData, ProdHeadCode: e.target.value })}
                    placeholder="Enter product head code"
                ...
                <div>
                  ...
                  <input
                    type="text"
                    required
                    value={formData.ProdHeadDesc}
                    onChange={(e) => setFormData({ ...formData, ProdHeadDesc: e.target.value })}
        ...
        <FilterPanel
          ...
          searchPlaceholder="Search by product head code or description..."
          filters={[
            {
              label: "Page Size",
              value: itemsPerPage,
        ...
        <div className="bg-white rounded-2xl shadow-xl shadow-slate-200/50 border border-slate-200 overflow-hidden">
          <div className="px-6 py-4 bg-gradient-to-r from-slate-50 to-slate-100 border-b border-slate-200 flex justify-between items-center">
            <h2 className="text-lg font-semibold text-slate-700">All Product Heads</h2>
          ...
          <div className="divide-y divide-slate-100">
            {paginatedProductHeads.map((ph) => (
              <div
                key={ph.ProdHeadCode}
                className="p-6 hover:bg-slate-50 transition-colors duration-200"
              >
                {editingCode === ph.ProdHeadCode ? (
                  /* Edit Mode */
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs font-semibold text-slate-500 uppercase mb-1">Product Head Code</label>
                        <input
                          type="text"
                          value={editFormData.ProdHeadCode}
                          disabled
                      ...
                      <div>
                        ...
                        <input
                          type="text"
                          value={editFormData.ProdHeadDesc}
                          onChange={(e) => setEditFormData({ ...editFormData, ProdHeadDesc: e.target.value })}
                    ...
                    <div className="flex gap-2 justify-end">
                      ...
                      <button
                        onClick={() => handleUpdate(ph.ProdHeadCode)}
                  ...
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => handleDelete(ph.ProdHeadCode)}
            ...
            {paginatedProductHeads.length === 0 && (
              <div className="p-12 text-center">
                <div className="w-16 h-16 bg-slate-100 rounded-full flex items-center justify-center mx-auto mb-4">
                  <Package className="w-8 h-8 text-slate-400" />
                </div>
                <h3 className="text-lg font-semibold text-slate-600 mb-2">No product heads found</h3>
                <p className="text-slate-500">Try adjusting your search query or add a new product head</p>
              </div>
            )}
          </div>
